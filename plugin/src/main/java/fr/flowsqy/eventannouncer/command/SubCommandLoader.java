...
public class SubCommandLoader {
    ...
    @NotNull
    public SubCommand[] load(@NotNull Plugin plugin, @NotNull MessageConfig messageConfig, @NotNull RootCommand rootCommand,
            @NotNull Map<String, InformationsData[]> sequences) {
        // Order does not matter but help should be the first for development consistency
        ...
        subCommands[1] = new SubCommand("send", new String[] { "s" }, "eventannouncer.command.send",
                messageConfig.getComponentMessage("command.help.send"),
